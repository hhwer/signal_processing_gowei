function [ quality,good_index, index_best5 ] = f2_neighbor( n1 ,opts)
%判断数据的质量
%   以相邻相似数据最多为优
%input
...
if nargin < 2
    ...
    if ~isfield(opts,'Max_continue_num')
        opts.Max_continue_num = 3;
    end
    if ~isfield(opts,'Total_num')
        opts.Total_num = 5;
    end
    
end
Max_continue_num = opts.Max_continue_num ;
Total_num = opts.Total_num;


index_best5 = zeros(Total_num,2);
name = [];
...
for n = n1:n1
    ...
    for i = 1:m
        ...
        elseif abs(local_sample(1)-local_sample(length(local_sample))) ...
            ...
            if  length(y) >=  (index(i+1)-index(i))/para_noise ||  length(b(b>para3)) >= (index(i+1)-index(i))/2
