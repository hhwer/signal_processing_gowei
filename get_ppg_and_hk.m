                ...
                for k = 1:size(a,1)
                    ...
                    for kk = 1:pulse_num
                        ...
                        if contains(string(name_i),id) && contains(string(name_i_ppg),id)
                            b = csvread([pulse_path,'/',pulse_i.name],1,0);
                            b1 = csvread([pulse_path_ppg,'/',pulse_i_ppg.name,1,0]);
                            custom = struct();
                            custom.customid = id;
                            custom.pweeks = pweeks;
                            custom_ppg = struct();
                            custom_ppg.customid = id;
                            custom_ppg.pweeks = pweeks;
                            if isempty(b)
                            ...
                            custom_ppg.pulse = b1(:,2);  
                            custom.pulse = b(:,3);  
                            
                            pulse_length = length(custom.pulse);
                            pulse_length_ppg = length(custom_ppg.pulse);
                            ...
                            opts.data={custom_ppg};
                            data1{data_num} = custom_ppg;
                            quality1 = f2_neighbor_low(1,opts); 
